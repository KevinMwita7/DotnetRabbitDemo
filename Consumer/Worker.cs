...
public class Worker : BackgroundService
{
        ...
        consumer.Received += async (ch, ea) =>
        {
            try {
                ...
                if (handleExceededRetryCount(ea)) {
                    ...
                    // to wait for manual intervention e.g. shovelling
                    ea.BasicProperties.ClearHeaders();

                    _channel.BasicPublish(QueueConfig.Exchange, QueueConfig.RoutingKeyP, true, ea.Body);
                } else {
                    // Right now just print the message. In real world scenarios, you might
                    // receive a JSON string which you can deserialize into POCO
    ...
    // Check if message has been retried more than thrice
